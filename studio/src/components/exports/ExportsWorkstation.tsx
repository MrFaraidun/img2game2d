...
export const ExportsWorkstation: React.FC<ExportsWorkstationProps> = ({
  ...
  /**
   * Trigger complete package download with real ZIP generation and audio feedback
   */
  const handleDownloadPackage = async () => {
    ...
    try {
      ...
      const engine = playground.activeEngine;

      // 1. Fetch each asset in manifestFiles and add to zip
      for (const item of manifestFiles) {
        try {
          ...
          if (res.ok) {
            ...
            console.warn(`Could not load ${item.downloadUrl}: status ${res.status}`);
          }
        } catch (err) {
          console.warn(`Failed to fetch ${item.downloadUrl}:`, err);
        }
      }

      // 2. Add the generated code script
      let scriptName = `${char}.gd`;
      if (engine === 'unity') {
        scriptName = `${activeChar.name.replace(/\s+/g, '')}Controller.cs`;
      } else if (engine === 'phaser') {
        scriptName = `${char}_scene.ts`;
      } else if (engine === 'pixijs') {
        scriptName = `${char}_sprite.ts`;
      }

      zip.file(scriptName, generatedCode);

      // 3. Add bundle README.md with clear integration instructions
      const readme = `# ${activeChar.name} - ${engine.toUpperCase()} Asset Bundle
Generated by img2game2d v3.0

- Target Engine: ${engine.toUpperCase()}
- Character: ${activeChar.name} (${char})

## Included Files:
${manifestFiles.map((f) => `- ${f.name} (${(f.sizeBytes / 1024).toFixed(1)} KB): ${f.description}`).join('\n')}
- ${scriptName}: Ready-to-use engine integration script

## Quick Start:
Drop this folder directly into your ${engine.toUpperCase()} project's asset directory.
`;
      ...
      a.href = url;
      a.download = `${char}_${engine}_bundle.zip`;
      ...
      <aside className="exports-sidebar">
        ...
        <div className="wb-panel-card bundle-action-card">
          ...
          >
            {isZipping ? (
              <>
                <Loader2 className="btn-icon-sm animate-spin" /> Packaging ZIP Bundle...
              </>
            ) : downloadSuccess ? (
              <>
                <Check className="btn-icon-sm" /> Bundle Downloaded!
              </>
            ) : (
              <>
                <FolderArchive className="btn-icon-sm" /> Download {playground.activeEngine.toUpperCase()} Bundle
              </>
            )}
          </button>
        </div>
